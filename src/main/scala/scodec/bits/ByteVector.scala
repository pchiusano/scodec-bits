...
sealed trait ByteVector extends BitwiseOperations[ByteVector,Int] with Serializable {
    ...
    if (n1 == size) ByteVector.empty
      ...
      def go(cur: ByteVector, n1: Int, accR: List[ByteVector]): ByteVector =
        cur match {
          ...
          case b: Buffer => accR.foldLeft(b.drop(n1))(_ ++ _)
          case c: Chunks => accR.foldLeft(c.drop(n1))(_ ++ _)
        }
      go(this, n1, Nil)
    }
  }
  ...
  def take(n: Int): ByteVector = {
    ...
    if (n1 == size) this
      ...
      def go(accL: ByteVector, cur: ByteVector, n1: Int): ByteVector = cur match {
        ...
        case c: Chunks => accL ++ c.take(n1)
        case b: Buffer => accL ++ b.take(n1)
      }
  ...
  private[scodec] final def foreachV(f: View => Unit): Unit = {
    ...
    def go(rem: List[ByteVector]): Unit = rem match {
      ...
      case Chunks(bs)  :: rem => go(bs.reverseIterator.foldLeft(rem)((tl,hd) => hd :: tl))
...
object ByteVector {
  ...
  extends ByteVector {
    ...
    def getImpl(n: Int): Byte = {
      ...
      chunks(i).getImpl(n - seen)
    }

    override def drop(n: Int): ByteVector =
      if (size <= n) ByteVector.empty
      else if (n <= 0) this
      else {
        var rem = n
        var lastSize = 0
        ...
        remChunks match {
          ...
          case _ => ByteVector.empty // should be caught by earlier `if`
        }
      }
  }
